...
class BasicAuth(Auth):
    ...
    def current_user(self, request=None) -> TypeVar('User'):
        ...
        value = self.authorization_header(request)
        print(value)
        extr_value = self.extract_base64_authorization_header(value)
        print(extr_value)
        decoded = self.decode_base64_authorization_header(extr_value)
        print(decoded)
        email, passwd = self.extract_user_credentials(decoded)
        print(decoded)
        self.user_object_from_credentials(email, passwd)
